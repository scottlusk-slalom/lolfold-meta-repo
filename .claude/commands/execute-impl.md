...
- **Zero new tests written**: HALT — this violates TDD protocol

### Pre-Submit Gate (MANDATORY — blocks PR)

Before execution can be considered complete, ALL of the following must pass:

1. **Build gate**: `npm run build` (or equivalent) exits 0
2. **Test gate**: `npm test` (or equivalent) exits 0
3. **New-test gate**: The diff MUST include new or modified test files covering the new code. If the diff contains new/modified source files but ZERO new/modified test files → HALT. This is not advisory — it is a hard stop.
4. **Coverage gate** (if `_loop-config.yaml` specifies `test.coverage_threshold`): Coverage on new files must meet threshold

If ANY gate fails, do NOT proceed to `/submit-pr`. Execution status remains incomplete.

### Completion
- All tasks marked `[x]` in impl-plan.md
- Full test suite passes
- Pre-submit gate passes (build + tests + new-test verification)
- Summary written to `_working/<key>/execution-summary.md`:
  - Tasks completed / total
  - Files created / modified
  - New test files added (list them)
...
## Key Constraints
- Tests MUST be written BEFORE implementation (strict TDD) — NO EXCEPTIONS
...
- A diff with zero new test files is NEVER acceptable — halt and write tests
- Pre-submit gate is mandatory — do not call `/submit-pr` until build, test, and new-test checks pass
