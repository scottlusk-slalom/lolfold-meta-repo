...
Gate transitions (`specified` → `planned` → `executed` → `submitted` → `archived`) are labelled automatically by `/update-gate`. Only `blocked` and `error` are set directly here since they don't map to gate transitions.
